...
int main()
{
  ...
  Shape const b_shape{.rows = COLS, .cols = ROWS};
  Tensor a(a_data, a_shape, serial_device);
  Tensor b(b_data, b_shape, serial_device);

  benchmark_mat_mul(serial_device, a, b);
  benchmark_mat_mul(eigen_device, a, b);
  benchmark_mat_mul(simd_device, a, b);
  benchmark_mat_mul(metal_device, a, b);
