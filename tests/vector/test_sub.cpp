...
void test_mat_sub(DevicePtr const &device, Tensor &a, Tensor &b, vec const &ref)
{
...
TEST_CASE("vector: sub", "[vector]")
{
  ...
  test_mat_sub(serial_device, a, b, ref);
  test_mat_sub(eigen_device, a, b, ref);
  test_mat_sub(simd_device, a, b, ref);
  test_mat_sub(metal_device, a, b, ref);
}
